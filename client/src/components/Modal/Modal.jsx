import { ModalContainer, ModalItem, ModalTitle,ColumnContainer, GeneralItem, GeneralTitle, CoinImg, CoinName, MiniCoinContainer, QuantityForm, QuantitySelector, QuantityLabel, BuyButton, SellButton, BalanceContainer, CoinPriceContainer } from "./ModalStyles"
import { useState } from "react"

const Modal = ({isOpen,onClose, modalAction, onBuyHandle, onSellHandle, singleCoin, balance, holdings}) => {
...
const userCoinHolding = holdings.find((holding) => holding.coin_id === singleCoin._id);
...
const remainingBalance = modalAction === "buy" ? balance - totalAmount : totalAmount + balance
...
console.log(userCoinHolding)
  ...
  return (
    ...
    <MiniCoinContainer>
      ...
      <GeneralItem>{userCoinQuantity} {singleCoin.name}</GeneralItem>
      </ColumnContainer>
      ...
      />
      {modalAction === "buy" ? <BuyButton disabled={quantity <= 0 || remainingBalance < 0}>Confirm Purchase</BuyButton> : <SellButton disabled={quantity <= 0 || userCoinQuantity< quantity}>Confirm Sale</SellButton>}
      </QuantityForm>
      <BalanceContainer>
      ...
      <ColumnContainer>
      <GeneralTitle>{modalAction === "buy" ? "Total Cost" : "Total Gain"}</GeneralTitle>
